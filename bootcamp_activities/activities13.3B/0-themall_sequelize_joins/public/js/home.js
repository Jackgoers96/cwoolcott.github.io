            ...
            function (response) {
                ...
                response.json().then(function (storeData) {
                    ...
                    for (var i = 0; i < storeData.length; i++) {
                        ...
                        openCloseBtn.attr("data-store-id", storeData[i].id);
                        openCloseBtn.attr("data-status", storeData[i].status);
                        openCloseBtn.html(storeData[i].status);

                        var store = $("<div>");
                        store.append(`<h3><a href="/store/${storeData[i].id}">${storeData[i].name}</a></h3>`);
                        store.append("<h4><i>" + storeData[i].description + "</i></h4>");
